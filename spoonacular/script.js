...
function renderMealList(category) {
  ...
  mealList.innerHTML = meal[category]
    .map(
      (item, index) => `
        <div>
...
function editMeal(category, index) {
  ...
  const newGrams = prompt(
    `Gib die neue Grammzahl für ${foodItem.name} ein:`,
    foodItem.grams
  );
...
function updateTotalNutritionAfterEdit(
  previousNutrition,
  newNutrition,
  category
) {
  totalNutrition[category].calories +=
    newNutrition.calories - previousNutrition.calories;
  totalNutrition[category].protein +=
    newNutrition.protein - previousNutrition.protein;
  totalNutrition[category].carbs +=
    newNutrition.carbs - previousNutrition.carbs;
  ...
  if (e.target && e.target.classList.contains("detail-btn")) {
    ...
    getFoodDetailsById(foodId).then((foodData) => {
      if (foodData) {
        ...
        selectedNutrients = {
          calories:
            foodData.nutrition.nutrients.find((n) => n.name === "Calories")
              ?.amount || 0,
          protein:
            foodData.nutrition.nutrients.find((n) => n.name === "Protein")
              ?.amount || 0,
          carbs:
            foodData.nutrition.nutrients.find((n) => n.name === "Carbohydrates")
              ?.amount || 0,
          fat:
            foodData.nutrition.nutrients.find((n) => n.name === "Fat")
              ?.amount || 0,
        };
        document.getElementById("detail-content").innerHTML = `
                    ...
                    <p><img src="https://spoonacular.com/cdn/ingredients_500x500/${foodData.image}" alt="${foodData.name}"></p>
                `;
